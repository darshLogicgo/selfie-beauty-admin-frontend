...
import { FolderOpen, Users, UserCheck, BarChart3, Loader2, Activity, CheckCircle2, XCircle, Clock, Filter, Calendar } from 'lucide-react';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { getDashboardStatsThunk, getLiveStatusThunk } from '@/store/dashboard/thunk';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

const Dashboard: React.FC = () => {
  const dispatch = useAppDispatch();
  const { stats, loading, liveStatus, liveStatusLoading } = useAppSelector((state) => state.Dashboard);
  ...
  const handleApplyCustomFilter = () => {
    ...
    if (end < start) {
      console.warn('End date is before start date, swapping dates', { startDate, endDate });
      // Swap dates if end is before start
      const temp = startDate;
      setStartDate(endDate);
      setEndDate(temp);
      dispatch(getLiveStatusThunk({ filter, startDate: endDate, endDate: startDate }));
      return;
    }
    
    // All validations passed, dispatch the action
    console.log('Applying custom filter with dates', { filter, startDate, endDate });
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-8">
            ...
            {filter === 'custom' && (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={!startDate || !endDate || (startDate && endDate && new Date(endDate) < new Date(startDate)) || startDate > today || endDate > today}
                  ...
                  title={!startDate || !endDate ? "Please select both dates" : (startDate > today || endDate > today ? "Cannot select future dates" : (startDate && endDate && new Date(endDate) < new Date(startDate) ? "End date must be after start date" : "Apply filter"))}
                >
                  Apply
                </button>
              </div>
        ...
        {liveStatusLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(liveStatus).map(([categoryName, categoryData], index) => {
              ...
              return (
                <div
                  ...
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <h3 className="font-semibold text-gray-900 mb-4 text-base">{categoryDisplayName}</h3>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-green-600 text-sm">{categoryData.success_rate}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-gray-900 text-sm">
                        {(categoryData.avg_response_time_ms / 1000).toFixed(2)}s
                      </span>
                    </div>
      ...
      <div className="stat-card">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-semibold text-foreground">Most Used Categories</h2>
        </div>
        ...
        <div className="space-y-4">
          {stats.mostUsedCategories && stats.mostUsedCategories.length > 0 ? (
            stats.mostUsedCategories.map((category, index) => {
              ...
              const percentage = maxUsers > 0 ? (category.mediaclicks / maxUsers) * 100 : 0;

              return (
                <div key={category._id} className="animate-slide-in" style={{ animationDelay: `${index * 100}ms` }}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-foreground">{category.name}</span>
                    <span className="text-muted-foreground text-sm">{category.mediaclicks.toLocaleString()} users</span>
                  </div>
