...
import { getHomeDataThunk, toggleHomeCategorySectionThunk, toggleHomeSubcategorySectionThunk } from "./thunk";
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getHomeDataThunk.fulfilled, (state, action) => {
      ...
      if (action.payload?.data) {
        state.data = {
          ...
          section6: action.payload.data.section6 || { title: "", categories: [] },
          section7: action.payload.data.section7 || { title: "", categories: [] },
        };
      }
    });
    builder.addCase(getHomeDataThunk.rejected, (state, action) => {
      state.loading = false;
      state.error = (action.payload as { message?: string })?.message || "Failed to fetch home data";
      state.message = "";
    });
    ...
    builder.addCase(toggleHomeCategorySectionThunk.fulfilled, (state, action) => {
      state.loading = false;
      const categoryIds = action.meta.arg.categories.map((cat) => cat._id);
      state.updatingIds = state.updatingIds.filter((id) => !categoryIds.includes(id));
      state.message = action.payload?.message || "";
      // Refresh data after successful toggle
      // The data will be updated via the updateSectionItem reducer calls
    });
    builder.addCase(toggleHomeCategorySectionThunk.rejected, (state, action) => {
      state.loading = false;
      const categoryIds = action.meta.arg.categories.map((cat) => cat._id);
      state.updatingIds = state.updatingIds.filter((id) => !categoryIds.includes(id));
      state.error = (action.payload as { message?: string })?.message || "Failed to update category section";
      state.message = "";
    });

    // =================================  Toggle Home Subcategory Section ==================================
    builder.addCase(toggleHomeSubcategorySectionThunk.pending, (state, action) => {
      state.loading = true;
      const subcategoryIds = action.meta.arg.subcategories.map((subcat) => subcat._id);
    ...
    builder.addCase(toggleHomeSubcategorySectionThunk.fulfilled, (state, action) => {
      state.loading = false;
      const subcategoryIds = action.meta.arg.subcategories.map((subcat) => subcat._id);
      state.updatingIds = state.updatingIds.filter((id) => !subcategoryIds.includes(id));
      state.message = action.payload?.message || "";
      // Refresh data after successful toggle
      // The data will be updated via the updateSectionItem reducer calls
    });
    builder.addCase(toggleHomeSubcategorySectionThunk.rejected, (state, action) => {
      state.loading = false;
      const subcategoryIds = action.meta.arg.subcategories.map((subcat) => subcat._id);
      state.updatingIds = state.updatingIds.filter((id) => !subcategoryIds.includes(id));
      state.error = (action.payload as { message?: string })?.message || "Failed to update subcategory section";
      state.message = "";
    });
  },
});
